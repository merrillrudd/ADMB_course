...
ad_comm::change_datafile_name(datafile);
  ...
  eyr.allocate("eyr");
  nage.allocate("nage");
  fwalpha.allocate("fwalpha");
  fwrho.allocate("fwrho");
  wla.allocate("wla");
  wlb.allocate("wlb");
  yt.allocate(syr,eyr,"yt");
  ct.allocate(syr,eyr,"ct");
  pat.allocate(syr,eyr,1,nage,"pat");
  iro.allocate("iro");
  icr.allocate("icr");
...
ghat=ighat;
  ...
  Zt.allocate(syr,eyr,1,nage,"Zt");
    ...
    Zt.initialize();
  #endif
  pahat.allocate(syr,eyr,1,nage,"pahat");
  #ifndef NO_AD_INITIALIZE
    pahat.initialize();
  #endif
}
...
void model_parameters::objective_function(void)
{
	...
	p_vec(1)=dbeta((h-0.2)/0.8,2,2);// put a weak beta prior on steepness
	if(last_phase())
	{
