rm(list=ls())
dir <- "C:\\merrill\\ADMB_course\\Day3\\3a_Uncertainty\\mcmc"

 FileN <- file.path(dir, "jump.txt",sep="")    


## selectivity model
Selex <-function(Length,A50,A95){
...
Test1 <-function()
{
 ...
 print(result)
 xfinal <- result$par
 Pred <- Selex(TheData$Length,exp(xfinal[1]),exp(xfinal[2]))
...
DoMCMC<-function(FileN, Xinit,Ndim,sd,cor,Nsim=1000,Nburn=0,Nthin=1)
{
...
# assign("co",co,pos=1)
 ...
 write(t(Outs2),paste(dir,"Res.Out",sep=""),ncolumns=4)
...
# Xinit <- c(log(40),log(60),-1)
 ...
 print(cor)
 DoMCMC(FileN, Xinit,3,sd,cor,Nsim=10000,Nburn=1,Nthin=5)
 
 Test2(paste(dir,"Res.Out",sep=""))
}
